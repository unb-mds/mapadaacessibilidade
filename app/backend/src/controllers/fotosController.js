  ...
  try {
    ...
    if (!Array.isArray(fotosData) || fotosData.length === 0) {
      return res.status(400).json({ 
        error: "O corpo da requisição deve ser um array de objetos de foto não vazio." 
      });
    }
    ...
    for (let i = 0; i < fotosData.length; i++) {
      ...
      if (!url || typeof url !== 'string' || url.trim() === '') {
        errors.push(`Foto ${i + 1}: URL é obrigatória e deve ser uma string válida`);
        continue;
      }

      if (!status || typeof status !== 'string' || status.trim() === '') {
        errors.push(`Foto ${i + 1}: Status é obrigatório e deve ser uma string válida`);
        continue;
      }

      if (!fk_usuario_id || typeof fk_usuario_id !== 'string' || fk_usuario_id.trim() === '') {
        errors.push(`Foto ${i + 1}: ID do usuário é obrigatório e deve ser uma string válida`);
        continue;
      }

      if (!fk_local_id || typeof fk_local_id !== 'string' || fk_local_id.trim() === '') {
        errors.push(`Foto ${i + 1}: ID do local é obrigatório e deve ser uma string válida`);
        continue;
      }
      ...
      try {
        ...
        errors.push(`Foto ${i + 1}: Erro ao salvar no banco - ${dbError.message}`);
      }
    }
